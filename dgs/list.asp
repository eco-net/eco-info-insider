...
rsPageData.Source = "SELECT m.id,m.zip,m.name,m.shortdescription,m.www,m.firstname,m.middlename,m.lastname,m.status,m.modidate,m.createdate,m.creator,m.maker,m.editor,stopped,m.fmpid FROM eiorg_maindata m  WHERE 0=0  ORDER BY m.name"
...
IF LEN(rsPageData.Fields.Item("name").Value)>2 THEN
	response.write rsPageData.Fields.Item("name").Value
ELSEIF LEN(rsPageData.Fields.Item("firstname").Value)>2 THEN
...
IF LEN(rsPageData.Fields.Item("firstname").Value)>0 AND LEN(rsPageData.Fields.Item("name").Value)>0 THEN
	response.write "<tr><td colspan='2'>"&rsPageData.Fields.Item("firstname").Value & "&nbsp;" & rsPageData.Fields.Item("middlename").Value & "&nbsp;" & rsPageData.Fields.Item("lastname").Value&"</td></tr>"
END IF
response.write "<tr><td nowrap><a href=""/dgb/new.asp?" & MM_keepBoth & MM_joinChar(MM_keepBoth) & "listoffset=" & MM_offset & "&orgid=" & rsPageData.Fields.Item("id").Value & """ title='Ny publikation'>Ny PUBL</a> "
response.write " | <a href=""/ok/new.asp?" & MM_keepBoth & MM_joinChar(MM_keepBoth) & "listoffset=" & MM_offset & "&orgid=" & rsPageData.Fields.Item("id").Value & """ title='Nyt arrangement'>Nyt ARR</a>"
'response.write " | <a href=""/kurser/new.asp?" & MM_keepBoth & MM_joinChar(MM_keepBoth) & "listoffset=" & MM_offset & "&orgid=" & rsPageData.Fields.Item("id").Value & """ title='Nyt kursus'>Nyt KURSUS</a>"
'response.write " | <a href=""/udd/new.asp?" & MM_keepBoth & MM_joinChar(MM_keepBoth) & "listoffset=" & MM_offset & "&orgid=" & rsPageData.Fields.Item("id").Value & """ title='Ny uddannelse'>Ny UDD</a>"
response.write " | <a href=""/art/new.asp?" & MM_keepBoth & MM_joinChar(MM_keepBoth) & "listoffset=" & MM_offset & "&orgid=" & rsPageData.Fields.Item("id").Value & """ title='Ny artikel'>Ny ART</a>"
response.write " | <a href=""javascript:;"" onclick=""window.open('changemailstatus.asp?orgid="&rsPageData.Fields.Item("id").Value&"','changemail','width=400,height=200,top=200,left=200')"">Mail</a>"
response.write "</td><td align='right' valign='top'><font color='red'>"
if request("valid")<>"" then
if rsPageData.Fields.Item("maker")<>""  then
response.write ("Oprettet: " & FormatDate(rsPageData.Fields.Item("createdate")) & " af " & rsPageData.Fields.Item("maker") & "</font><br>")
if rsPageData.Fields.Item("editor")<>"" then 
response.write ("<font color='red'>Rettet: " & FormatDate(rsPageData.Fields.Item("modidate")) & " af " & rsPageData.Fields.Item("editor") & "</font>")
end if
else 'maker/editor
if len(rsPageData.Fields.Item("status"))>0 then
	if rsPageData.Fields.Item("status")=1 then 
	response.write ("&nbsp;&nbsp;Oprettet af Øko-net " & FormatDate(rsPageData.Fields.Item("createdate")))
	elseif rsPageData.Fields.Item("status")=2 then 
	response.write ("&nbsp;&nbsp;Oprettet af Bruger " & FormatDate(rsPageData.Fields.Item("createdate")))
	elseif rsPageData.Fields.Item("status")=3 then 
	response.write ("&nbsp;&nbsp;Rettet af Øko-net " & FormatDate(rsPageData.Fields.Item("modidate")))
		if Len(rsPageData.Fields.Item("creator"))>0 then
			if rsPageData.Fields.Item("creator")=2 then
				response.write("</font><br>Oprettet af bruger: " & FormatDate(rsPageData.Fields.Item("createdate")) & "<font color='red'>")
			else
				response.write("</font><br>Oprettet af Øko-net: " & FormatDate(rsPageData.Fields.Item("createdate")) & "<font color='red'>")
			end if
		end if
	elseif rsPageData.Fields.Item("status")=4 then 
		response.write ("&nbsp;&nbsp;Rettet af Bruger " & FormatDate(rsPageData.Fields.Item("modidate")))
		if Len(rsPageData.Fields.Item("creator"))>0 then
			if rsPageData.Fields.Item("creator")=2 then
				response.write("</font><br>Oprettet af bruger: " & FormatDate(rsPageData.Fields.Item("createdate")) & "<font color='red'>")
			else
