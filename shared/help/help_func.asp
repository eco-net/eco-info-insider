...
Dim rsFolder__theID
rsFolder__theID = "0"
if (request("func")     <> "") then rsFolder__theID = request("func")    
%>
<%
set rsFolder = Server.CreateObject("ADODB.Recordset")
rsFolder.ActiveConnection = MM_ecoinfo_STRING
rsFolder.Source = "SELECT *  FROM eihelp_detail  WHERE func_id=" + Replace(rsFolder__theID, "'", "''") + "  ORDER BY name"
...
Dim rsSek__theID
rsSek__theID = "0"
if (request("func")   <> "") then rsSek__theID = request("func")  
%>
<%
set rsSek = Server.CreateObject("ADODB.Recordset")
rsSek.ActiveConnection = MM_ecoinfo_STRING
rsSek.Source = "SELECT *  FROM eihelp_func  WHERE id=" + Replace(rsSek__theID, "'", "''") + ""
rsSek.CursorType = 0
...
rsText.Source = replace(rsText.Source,"0=0","r.f=" & request("func"))
...
<tr> 
<td valign="top" colspan="3" class="listheader" height="20" bgcolor="#eeeeee"><%=request("secname")%>/<%=request("brname")%><br>
...
%>
<a href="help_detail.asp?sec=<%=request("sec")%>&secname=<%=request("secname")%>&br=<%=request("br")%>&brname=<%=request("brname")%>&func=<%=request("func")%>&funcname=<%=rsSek("name")%>&detail=<%=(rsFolder.Fields.Item("id").Value)%>"><%=(rsFolder.Fields.Item("name").Value)%></a> <br>
<% 
...
<a href="help_func.asp?sec=<%=request("sec")%>&br=<%=request("br")%>&secname=<%=request("secname")%>&brname=<%=request("brname")%>&func=<%=request("func")%>&funcname=<%=request("funcname")%>&tekst=<%=(rsText.Fields.Item("id").Value)%>">>><%=(rsText.Fields.Item("name").Value)%></a> 
