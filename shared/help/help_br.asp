...
Dim rsFolder__theID
rsFolder__theID = "0"
if (request("br")    <> "") then rsFolder__theID = request("br")   
%>
<%
set rsFolder = Server.CreateObject("ADODB.Recordset")
rsFolder.ActiveConnection = MM_ecoinfo_STRING
rsFolder.Source = "SELECT *  FROM eihelp_func  WHERE br_id=" + Replace(rsFolder__theID, "'", "''") + "  ORDER BY name"
...
Dim rsSek__theID
rsSek__theID = "0"
if (request("br")   <> "") then rsSek__theID = request("br")  
%>
<%
set rsSek = Server.CreateObject("ADODB.Recordset")
rsSek.ActiveConnection = MM_ecoinfo_STRING
rsSek.Source = "SELECT *  FROM eihelp_branch  WHERE id=" + Replace(rsSek__theID, "'", "''") + ""
rsSek.CursorType = 0
...
if request("br")<>"" then
rsText.Source = replace(rsText.Source,"0=0","r.b=" & request("br"))
...
<tr> 
<td valign="top" colspan="3" class="listheader" height="20" bgcolor="#eeeeee"><%=request("secname")%>/<br>
...
%>
<a href="help_func.asp?sec=<%=request("sec")%>&secname=<%=request("secname")%>&br=<%=request("br")%>&brname=<%=rsSek("name")%>&func=<%=(rsFolder.Fields.Item("id").Value)%>"><%=(rsFolder.Fields.Item("name").Value)%></a> <br>
<% 
...
<a href="help_br.asp?sec=<%=request("sec")%>&br=<%=request("br")%>&secname=<%=request("secname")%>&brname=<%=request("brname")%>&func=<%=request("func")%>&tekst=<%=(rsText.Fields.Item("id").Value)%>"><%=(rsText.Fields.Item("name").Value)%></a> 
