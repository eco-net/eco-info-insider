...
rsPageData.Source = "SELECT m.id,m.title,m.shortdescription,m.www,m.startdate,m.enddate,m.status,m.modidate,p.city,m.maker,m.editor,m.createdate  FROM (eicourse_maindata m LEFT JOIN eisys_postnr p ON m.postnr=p.postnr)  WHERE 0=0  ORDER BY m.startdate"
rsPageData.Source=replace(rsPageData.Source,"0=0",theWhere)
rsPageData.Source=replace(rsPageData.Source,"(eicourse_maindata m LEFT JOIN eisys_postnr p ON m.postnr=p.postnr)",theFrom)
...
<%ELSE%>
<!--include file="inc_info.asp"--><!--Bruger indgang er deaktiveret-->
<%END IF%>
<!-- #EndEditable --></td>
...
<table width="100%" border="0" cellspacing="5">
  <tr>
    <td><p>&nbsp;</p>
      <p><strong>Kursus-og uddannelseskalender er deaktiveret</strong></p>
      <p>Kontakt &Oslash;ko-net hvis du har behov for at bruge disse.  </p></td>
  </tr></table>
<!--Bruger indgang er deaktiveret:-->
...
<td class="contentHeader1" colspan="2" width="98%">Fandt <%=rsPageData_total%> kurser <font color="#FF0000"><%=validtext%></font></td>
...
response.write "</a></span>&nbsp;&nbsp;&nbsp;<a href=""" & replace(replace(detailLink,"##",rsPageData("id")),"edit","delete") & """>Slet</a><br>"
...
if len(rsPageData.Fields.Item("status"))>0 then
	...
	response.write FormatDate(rsPageData.Fields.Item("modidate"))
end if
end if
end if
...
<td class="contentHeader1" height="22">Fandt ingen kurser</td>
<td width="8"><br>
</td>
</tr>
<tr> 
...
<br>
<br>
Der er ingen kurser, der matcher din søgning.<br>
