...
rsPageData.Source = "SELECT o.id, o.name, o.fullname, s.username,s.page,s.date,s.app_id,s.sek,s.app_title  FROM eiorg_maindata o INNER JOIN eisys_insideruser_stat s ON  o.id=s.orgid ORDER BY date desc"
...
%>
<html><!-- #BeginTemplate "/Templates/1cols.dwt" -->
<head>
...
Brugere der har rettet siden 25/11-02: 
...
<td width="15%" class="sitetag" nowrap><%=Datepart("d",(rsPageData.Fields.Item("date").Value))%>/<%=Datepart("m",(rsPageData.Fields.Item("date").Value))%>-<%=(datepart("yyyy",rsPageData.Fields.Item("date").Value)) %> </td>
</tr>
<% 
