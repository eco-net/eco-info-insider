...
function record(theform) {
	...
	else if(elementExsists(theform,'selCat')==true && theform.selCat.options.length==0)alert('Der skal vælges kategorier.')
	...
	else if(elementExsists(theform,'username')==true && (theform.username.value.length>20 || theform.username.value.length<4))alert('Brugernavn skal være på mellem 4 og 20 tegn')
	else if(elementExsists(theform,'password')==true && (theform.password.value.length>8 || theform.password.value.length<4))alert('Adgangskode skal være på mellem 4 og 8 tegn')
	else {
