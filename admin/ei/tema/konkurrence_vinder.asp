...
rsChosen.Source = "SELECT id, name  FROM eiafstem_emne  WHERE chosen = 1"
...
<%
Dim chosenID
chosenID=CStr(rsChosen("id"))
%>
<%
Dim theWinners,gamblers,winnr,winner
theWinners=""
...
<%
Function draw()
set rsKonkurrence = Server.CreateObject("ADODB.Recordset")
rsKonkurrence.ActiveConnection = MM_ecoinfo_STRING
rsKonkurrence.Source = "SELECT *  FROM eiafstem_konkurrence WHERE emne_id=" & chosenID
rsKonkurrence.CursorType = 0
rsKonkurrence.CursorLocation = 2
rsKonkurrence.LockType = 3
rsKonkurrence.Open()
gamblers=0
while not rsKonkurrence.EOF
gamblers=gamblers+1
rsKonkurrence.MoveNext
wend
rsKonkurrence.MoveFirst
winnr=randomnumber

for i=1  to winnr
rsKonkurrence.MoveNext
next
winner="<a href='mailto:" & rsKonkurrence("mail") & "'>" & rsKonkurrence("mail") & "</a>"

rsKonkurrence.Close()
theWinners=request("theWinners") & "<br>" & winner
...
MySec = Second(Now)
Randomize(MySec)
randomnumber=CInt(Rnd()* (gamblers-1)) +1
...
</tr></table><!-- #EndLibraryItem --></p><!-- #EndEditable --> 
...
<tr> 
<td colspan="2" bgcolor="#CCCCCC"><span class="sidebarHeader">Find en vinder af 
den aktuelle konkurrence</span><span class="contentHeader2"></span></td>
</tr>
</table>
...
<tr> 
<td class="contentHeader2" width="47%"> 
<p>&nbsp;</p>
<p align="center">Afstemningen:<br>
<%=(rsChosen.Fields.Item("name").Value)%></p>
<p align="center">&nbsp;</p>
</td>
<td width="53%"> 
<form name="form1" method="post" action="konkurrence_vinder.asp">
<p class="listheader" align="center">Udtr&aelig;k een vinder</p>
<p align="center"> 
<input type="submit" name="Submit" value="Tr&aelig;k" class="formSubmitbutton">
<input type="hidden" name="draw" value="1">
<input type="hidden" name="theWinners" value="<%=theWinners%>">
</p>
</form>
</td>
</tr>
...
<p align="center"><span class="listheader">And the winners are:<br>
</span>der kan forekomme dubletter!</p>
<p class="listheader" align="center"><font color="red"><%=theWinners%></font></p>
</td>
</tr>
</table>
</td>
