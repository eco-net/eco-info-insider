...
<%
if request("h")<>"" then
	theSQL="UPDATE eitema_maindata SET header='" & request("header") & "', subheader='" & request("subheader") & "' WHERE id=" & request("id")
	execCommand(theSQL)
end if
%>
<%
...
If (CStr(Request("MM_insert")) <> "") Then
  ...
  MM_editTable = "eitema_artikel"
  MM_editRedirectUrl = ""
  MM_fieldsStr  = "title|value|tekst|value|linktekst|value|link|value|tema_id|value|place|value"
  MM_columnsStr = "title|',none,''|tekst|',none,''|linktext|',none,''|link|',none,''|tema_id|none,none,NULL|place|none,none,NULL"
...
%>
<%
Dim rsArtikel__temaid
rsArtikel__temaid = "0"
if (request("id") <> "") then rsArtikel__temaid = request("id")
%>
<%
set rsArtikel = Server.CreateObject("ADODB.Recordset")
rsArtikel.ActiveConnection = MM_ecoinfo_STRING
rsArtikel.Source = "SELECT *  FROM eitema_artikel  WHERE tema_id=" + Replace(rsArtikel__temaid, "'", "''") + "  ORDER BY place"
...
rsTema.Source = "SELECT id, header, subheader  FROM eitema_maindata  WHERE id=" + Replace(rsTema__theID, "'", "''") + ""
rsTema.CursorType = 0
...
Dim Repeat1__index
Repeat1__index = 0
rsArtikel_numRows = rsArtikel_numRows + Repeat1__numRows
%>
<!--#include virtual="/shared/common.asp" -->
<%
Dim id, name
...
<tr> 
<td colspan="2" bgcolor="#CCCCCC"><span class="sidebarHeader">Rediger Tema </span><span class="contentHeader2"><%=name%></span></td>
</tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr> 
<td valign="top"><span class="contentHeader2">Artikler p&aring; Temaside</span><br>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr> 
...
<tr> 
<td> 
<form name="form3" method="post" action="edit_artikler.asp?h=<%=id%>">
<div align="center"> <span class="listheader">Overskrift p&aring; temaside</span> 
<input type="text" name="header" class="formInputobjectLong" value="<%=(rsTema.Fields.Item("header").Value)%>">
<br>
<span class="listheader">Underoverskrift p&aring; temaside</span> 
<input value="<%=(rsTema.Fields.Item("subheader").Value)%>" type="text" name="subheader" class="formInputobjectLong">
<br>
<input type="hidden" name="id" value="<%=request("id")%>">
<input type="submit" name="Submit3" value="Gem" class="formSubmitbutton">
</div>
</form>
</td>
</tr>
...
<tr> 
<td> 
<p align="center"><span class="listheader"> Titel</span> <br>
<span class="listheader">Ny Artikel</span> 
<input type="text" name="title" class="formInputobject">
<br>
<textarea name="tekst" class="formTextobjectBigger"></textarea>
&nbsp;<br>
<span class="formLabeltext">linktekst: </span> 
<input type="text" name="linktekst" value="l&aelig;s mere..." class="formInputobject">
<span class="formLabeltext"> <br>
link(http://www.link.dk): </span> 
<input type="text" name="link" class="formInputobject">
</p>
<p align="center"> 
<input type="hidden" name="tema_id" value="<%=id%>">
placering 
<input type="text" name="place" size="2">
...
<% 
While ((Repeat1__numRows <> 0) AND (NOT rsArtikel.EOF)) 
%>
<table width="90%" border="0" cellspacing="0" cellpadding="0" align="center">
<tr> 
<td> 
...
<td> 
<p align="center"><span class="listheader">Titel</span> <br>
<span class="listheader">Rediger Artikel</span> 
<input value="<%=(rsArtikel.Fields.Item("title").Value)%>" type="text" name="title" class="formInputobject">
<br>
<textarea name="tekst" class="formTextobjectBigger"><%=(rsArtikel.Fields.Item("tekst").Value)%></textarea>
&nbsp;<br>
<span class="formLabeltext">linktekst:</span> 
<input type="text" name="linktekst" value="<%=(rsArtikel.Fields.Item("linktext").Value)%>" class="formInputobject">
<span class="formLabeltext"> <br>
link(http://www.link.dk):</span> 
<input value="<%=(rsArtikel.Fields.Item("link").Value)%>" type="text" name="link" class="formInputobject">
</p>
<p align="center"> 
<input type="hidden" name="tema_id" value="<%=id%>">
placering 
<input value="<%=(rsArtikel.Fields.Item("place").Value)%>" type="text" name="place" size="2">
