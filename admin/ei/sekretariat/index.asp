...
If (CStr(Request("MM_insert")) <> "") Then
  ...
  MM_editTable = "sekretariat"
  MM_editRedirectUrl = ""
  MM_fieldsStr  = "navn|value|stilling|value|email|value"
  MM_columnsStr = "navn|',none,''|stilling|',none,''|email|',none,''"
...
IF request("doUpdate")=1 THEN
	...
	FOR each item in request("id")
		theSQL=theSQL & "UPDATE sekretariat SET navn='" & request("navn")(i) & "',stilling='" & request("stilling")(i)  & "',email='" & request("email")(i) & "',orden=" & CInt(request("orden")(i)) & " WHERE id=" & item & ";" & chr(13)
		i=i+1
	NEXT
	DIM theComm
    ...
    theComm.ActiveConnection = MM_ecoinfo_STRING
 	IF LEN(request("delete"))>0 THEN
		theComm.CommandText="Delete FROM sekretariat WHERE id IN (" & request("delete") & ")"
		theComm.Execute
	END IF
...
<p>&nbsp;</p>
<p><a href="/admin/ei/bestyrelse/index.asp">Bestyrelse</a></p>
<!-- #EndEditable --></td>
...
<p>&nbsp;</p>
<p><span class="contentHeader1">Sekretariat </span><br>
</p>
...
<input type="checkbox" name="delete" value="<%=(rsPageData.Fields.Item("id").Value)%>">
</td>
</tr>
<% 
...
<td> 
<input type="text" name="stilling" class="formInputobjectLong">
</td>
<td> 
