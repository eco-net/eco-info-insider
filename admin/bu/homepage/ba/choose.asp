...
<%
set rsPageelements = Server.CreateObject("ADODB.Recordset")
rsPageelements.ActiveConnection = MM_ecoinfo_STRING
rsPageelements.Source = "SELECT id  FROM eiba_maindata WHERE siteid=2 AND onhomepage=1"
rsPageelements.CursorType = 0
rsPageelements.CursorLocation = 2
rsPageelements.LockType = 3
rsPageelements.Open()
rsPageelements_numRows = 0

DIM firsttime
IF rsPageelements.EOF then firsttime=1 else firsttime=0
%>
...
<form name="form1" method="post" action="" onSubmit="dochoose(document.forms[0]);return document.validation;">
...
<div align="center"><span class="contentHeader1">V&aelig;lg Branding Area til 
Forsiden</span><span></span></div>
</td>
</tr>
<tr> 
<td> <span class="formLabeltext">Branding Area: </span><br>
<select name="ba1" class="formInputobjectLong">
<%
IF firsttime=1 THEN
	filename=Server.MapPath("/log/insider/bu/menufiles/sel_ba.txt")
ELSE
	cursel=rsPageelements.Fields.Item("id").value
	filename=Server.MapPath("/log/insider/bu/menufiles/sel_ba_dyn.txt")
END IF
SET fs = CreateObject("Scripting.FileSystemObject")
IF fs.FileExists(filename) THEN
	...
	ts=""
ELSE
	response.write "<option value=""0"">&lt;ingen emner&gt;</option>"
END IF
fs=""
%>
</select>
...
<br>
<input type="button" name="Submit2" value="Fortryd" class="formbutton" onClick="this.form.action='/admin/bu/functions.asp';this.form.submit();">
<input type="submit" name="Submit" value="OK" class="formSubmitbutton">
<br>
<br>
</td>
</tr>
